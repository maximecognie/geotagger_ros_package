...
class mapping_hexa:
  ...
  def __init__(self):
    ...
    self.image_sub = message_filters.Subscriber("/recovered_stream_from_listener", Image)
  ...
  def make_image(self, img_data, gps_data):
    ...
    picNameTimeString = datetime.utcfromtimestamp(picGpsTimeFlt).strftime('%Y_%m_%d_T_%H_%M_%S')
    picSaveName = ""
    ...
    #self.set_gps_location(picPath, lat=gps_data.latitude, lng=gps_data.longitude,altitude=gps_data.altitude, gpsTime=gpsTimeString)
